...
def processDomain(dns_server_ip, domain_name):
	...
	while loop < noOfIp:
		...
		result = os.system("ping -n 1 " + app_Server_ip)
		if result < 1:
			is_alive = 1;
		if result > 0:
			is_alive = 0;
		data = { 'dns_ip': dns_server_ip, 'app_server_ip': app_Server_ip, 'domain_name': domain_name, 'is_alive': is_alive, 'type': 'icmp_ping', 'req_id': request_id }
		try:
